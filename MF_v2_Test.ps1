# 自动开始生产模式（test）打包，在全部完成后，将各个远端模块提供者项目（子项目）的输出复制到远端模块使用者项目（主项目）的输出目录中。

chcp 65001

Write-Host "`n`n自动开始生产模式（test）打包，在全部完成后，将各个远端模块提供者项目（子项目）的输出复制到远端模块使用者项目（主项目）的输出目录中。`n`n`n`n"
...
function StartRun
{
    foreach ($innerArray in $nestedArray)
    {
        try
        {
            ...
            Write-Host "`n`n`n`n------------"$innerArray[0]"------生产模式（test）打包完成:"$innerArray[1]"------------`n`n`n`n"
        ...
        catch
        {
            ...
            Write-Host "发生生产模式（test）打包错误的项目："
...
if (StartRun)
{
    Write-Host "`n`n已完成全部的生产模式（test）打包！`n`n"
