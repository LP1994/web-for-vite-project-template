 ...
 * FileDirPath: simulation_servers/deno/test/mongodb_demo/npm_mongoose_for_node.test.mts
 * Author: 12278
...
/**
 * 2024年10月30日，基于：npm包mongoose@8.7.3（该版本的mongoose是基于npm包mongodb@6.9.0）、mongodb-windows-x86_64-enterprise-8.0.3、npm@10.9.0、node@23.1.0，带客户端证书的与数据库进行的TLS连接测试成功！
 * 注意该文件不是deno版！
 */
...
const mongooseClientConfig: T_ConnectOptions = {
   ...
   * 该选项值在用作配置选项连接数据库时，值保持这样的字符串格式即可：'G:\\WebStormWS\\web-for-vite-project-template\\simulation_servers\\deno\\openssl\\MongoDBSSL001\\001根CA证书\\MongoDBSSL001_Root_CA.pem'。<br />
   * 但是作为连接字符串时，tlsCAFile选项的值需要设置成：encodeURIComponent( 'G:\\WebStormWS\\web-for-vite-project-template\\simulation_servers\\deno\\openssl\\MongoDBSSL001\\001根CA证书\\MongoDBSSL001_Root_CA.pem' )。<br />
   * 作为连接字符串时，tlsCAFile选项的值总是需要被encodeURIComponent()调用后返回的。
   *
   * @type {string}
   */
  tlsCAFile: 'G:\\WebStormWS\\web-for-vite-project-template\\simulation_servers\\deno\\openssl\\MongoDBSSL001\\001根CA证书\\MongoDBSSL001_Root_CA.pem',
  /**
   ...
   * 该选项值在用作配置选项连接数据库时，值保持这样的字符串格式即可：'G:\\WebStormWS\\web-for-vite-project-template\\simulation_servers\\deno\\openssl\\MongoDBSSL001\\004客户端CA证书\\MongoDBSSL001_Clients_CA.pem'。<br />
   * 但是作为连接字符串时，tlsCertificateKeyFile选项的值需要设置成：encodeURIComponent( 'G:\\WebStormWS\\web-for-vite-project-template\\simulation_servers\\deno\\openssl\\MongoDBSSL001\\004客户端CA证书\\MongoDBSSL001_Clients_CA.pem' )。<br />
   * 作为连接字符串时，tlsCertificateKeyFile选项的值总是需要被encodeURIComponent()调用后返回的。
   *
   * @type {string}
   */
  tlsCertificateKeyFile: 'G:\\WebStormWS\\web-for-vite-project-template\\simulation_servers\\deno\\openssl\\MongoDBSSL001\\004客户端CA证书\\MongoDBSSL001_Clients_CA.pem',
  /**
  ...
  /**
   * 指定本地TLS证书的位置。<br />
   * 该选项值在用作配置选项连接数据库时，值保持这样的字符串格式即可：'G:\\WebStormWS\\web-for-vite-project-template\\simulation_servers\\deno\\openssl\\MongoDBSSL001\\004客户端CA证书\\MongoDBSSL001_Clients_CA.crt'。<br />
   * 但是作为连接字符串时，tlsCertificateFile选项的值需要设置成：encodeURIComponent( 'G:\\WebStormWS\\web-for-vite-project-template\\simulation_servers\\deno\\openssl\\MongoDBSSL001\\004客户端CA证书\\MongoDBSSL001_Clients_CA.crt' )。<br />
...
async function run(): Promise<void>{
  try{
    ...
    const startup_log: Array<I_StartupLogCollectionSchema> = await ( await startup_log_collection.find<I_StartupLogCollectionSchema>() ).toArray();
