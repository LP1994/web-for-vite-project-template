...
const browserslist = [
    ...
    // PC端各主流浏览器的最新版本，至20240618。Start
    'Chrome >= 126',
    ...
    'Safari >= 17',
    'Opera >= 111',
    // PC端各主流浏览器的最新版本，至20240618。End

    // 移动端各主流浏览器的最新版本，至20240618。Start
    'ChromeAndroid >= 126',
    // 从Android 4.4后Android WebView直接跟Chrome同步。
    'Android >= 126',
    'FirefoxAndroid >= 128',
    'iOS >= 17',
    // 移动端各主流浏览器的最新版本，至20240618。End
  ],
  /**
    ...
    esbuild = {
      ...
      tsconfigRaw: ( tsconfigPath => {
        ...
        while( 'extends' in obj1 && obj1.extends.length !== 0 ){
          ...
          resultCompilerOptionsObj = Object.assign(
            {},
            Object.prototype.toString.call( obj1.compilerOptions ) === '[object Object]'
            ? JSON.parse( JSON.stringify( obj1.compilerOptions ) )
            : {},
    ...
    css: {
      ...
      modules: {
        ...
        getJSON: async ( cssFileName, json, outputFileName ) => {
          await writeFile( resolve( __dirname, `./src/static/json/css_modules/${ basename( cssFileName ).replace( new URL( cssFileName ).search, '' ) }.css.modules.json` ), JSON.stringify( json ), {
            flag: 'w+',
          } );
        },
        /**
  ...
  if( command === 'serve' ){
    return {
      ...
      server: {
        ...
        /**
         * 为开发服务器配置CORS。这在默认情况下是启用的，允许任何来源。传递一个选项对象来微调行为，或传递false来禁用。<br />
         * 详细见：<br />
        ...
        cors: {
           ...
           */
          origin: ( origin => origin.split( ',' ).map( item => item.trim() ) )( HttpResponseHeadersFun()[ 'Access-Control-Allow-Origin' ] ),
          /**
           * @type {string | string[]} 配置Access-Control-Allow-Methods CORS头。希望是一个以逗号分隔的字符串（例如：'GET,PUT,POST'）或一个数组（例如：['GET', 'PUT', 'POST']）。
           */
          methods: ( methods => methods.split( ',' ).map( item => item.trim() ) )( HttpResponseHeadersFun()[ 'Access-Control-Allow-Methods' ] ),
          /**
           ...
           */
          allowedHeaders: ( allowedHeaders => allowedHeaders.split( ',' ).map( item => item.trim() ) )( HttpResponseHeadersFun()[ 'Access-Control-Allow-Headers' ] ),
          /**
           ...
           */
          exposedHeaders: ( exposedHeaders => exposedHeaders.split( ',' ).map( item => item.trim() ) )( HttpResponseHeadersFun()[ 'Access-Control-Expose-Headers' ] ),
          /**
           * @type {boolean} 配置Access-Control-Allow-Credentials CORS头。设置为true以传递该头信息，否则就省略。
           */
          credentials: HttpResponseHeadersFun()[ 'Access-Control-Allow-Credentials' ],
          /**
           * @type {number} 配置Access-Control-Max-Age CORS头。设置为整数以通过该头，否则省略。
           */
          maxAge: HttpResponseHeadersFun()[ 'Access-Control-Max-Age' ],
          /**
